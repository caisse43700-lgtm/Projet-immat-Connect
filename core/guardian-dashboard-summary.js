...
(function(w){
  ...
  function _installHeaderActions(){
    if($('guardianDiagToggleBtn')) return true;
    var rightGroup = document.getElementById('gardienHeaderRight');
    if(!rightGroup) return false;
    var immatest = document.getElementById('btnImmatestTab');
    ...
    [
      {id:'guardianDiagToggleBtn', style:btnStyle, text:'Diagnostic'},
      {id:'guardianCopyReportBtn', style:btnStyle, text:'Copier'},
      {id:'guardianGlobalCheckBtn', style:globalBtnStyle, text:'Global'},
      {id:'guardianMajBtn', style:majBtnStyle, text:'🔄 MAJ', onclick:'window._forceSwUpdate&&window._forceSwUpdate()'},
    ].forEach(function(b){
      var btn = document.createElement('button');
      btn.id = b.id; btn.type = 'button'; btn.style.cssText = b.style; btn.textContent = b.text;
      if(b.onclick) btn.setAttribute('onclick', b.onclick);
      rightGroup.insertBefore(btn, immatest);
    });
    return true;
  }

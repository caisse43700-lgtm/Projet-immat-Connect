...
(function () {
  ...
  ok('« Ange » redit en conversation → ré-accueil (jamais LLM)', /anges\?\|l'ange\|lange\|angel\|angie\)\[\\s,.!\?\]\*\$\//.test(HTML) && HTML.includes("this._speakRetry('Oui, je t\\'écoute')"));
  ...
  ok('speak() trace l\'état réel de la voix (starting/speaking/done)', /window\._icTts='starting'/.test(HTML) && /u\.onstart=function\(\)\{if\(window\._icTtsU===u\)window\._icTts='speaking';\}/.test(HTML));
  ok('garde d\'instance TTS (vieille phrase ne clobber pas)', /window\._icTtsU=u/.test(HTML));
  ...
  ok('GPS vocal via navigate', /App\.navigate\('gps',\{source:'voice'\}\)/.test(HTML));
  ok('Réglages vocal via navigate', /App\.navigate\('reglages',\{source:'voice'\}\)/.test(HTML));
  ok('invariant en PROD : tolérance 2 ticks + correction vers l\'intention', HTML.includes('panneaux visibles') && /this\._visViol/.test(HTML) && /App\._lastNavIntent&&App\._lastNavIntent\.pid/.test(HTML));
  ...
  ok('navAppels force openSheet', /App\.navAppels=function\(\)\{[\s\S]{0,1200}panel\('messages'\);App\._forcePanel\?\.\('panelMessages'\)/.test(HTML));
  ...
  ok('catégorie signaler via navigate (step)', /App\.navigate\('signaler',\{source:'voice',step:f\}\)/.test(HTML));
  ...
  ok('_tryCategory → défaut = activité via navigate (category)', /App\.navigate\('activite',\{source:'voice',category:ac\}\)/.test(HTML));
  ...
  ok('_tryOpen ouvre via le propriétaire unique navigate()', /App\.navigate\('messages',\{source:'voice'\}\)/.test(HTML) && /App\.navigate\('appels',\{source:'voice'/.test(HTML));

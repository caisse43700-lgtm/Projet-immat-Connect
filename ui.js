...
(function () {
  ...
  function loadScriptOnce(src){
    return new Promise((resolve, reject) => {
      ...
      s.async = true;
      s.onload = resolve;
      s.onerror = () => reject(new Error('Chargement Supabase impossible'));
  ...
  async function ensureSupabase(){
    ...
    try{
      await loadScriptOnce('https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2.49.4/dist/umd/supabase.min.js');
    }catch(e){
      console.warn('[UI bridge] Supabase CDN load failed', e);
    }
    if (window.supabase && window.supabase.createClient) {
      window.sb = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY);
      return window.sb;
  ...
  function showAuth(mode){
    ...
    }catch(e){ console.warn('[UI bridge] App.goAuth failed, fallback auth screen', e); }
  ...
  async function fallbackLogin(){
    ...
    if (!sb || !sb.auth || typeof sb.auth.signInWithPassword !== 'function') {
      showStatus('Supabase indisponible : CDN ou initialisation bloquée.', 'error');
      unlockAuth();
      return;
    }
    ...
    try{
      ...
      showStatus(msg.includes('Invalid login') || msg.includes('Invalid credentials') ? 'Email ou mot de passe incorrect.' : msg, 'error');
    }finally{ unlockAuth(); }
  }
  ...
  async function submitAuth(){
    exposeAppIfPossible();
    try{
      if (window.App && typeof window.App.handleAuth === 'function') { await window.App.handleAuth(); return; }
      if (window.App && typeof window.App.auth === 'function') { await window.App.auth(); return; }
    }catch(e){ console.warn('[UI bridge] App auth failed, trying fallback login', e); }
    await fallbackLogin();
  }
  ...
  function bindAuthButton(){
    ...
    if (b && !b.__safeAuthBridge) {
      ...
      b.addEventListener('click', function(e){ e.preventDefault(); e.stopPropagation(); submitAuth(); }, true);
    }
  }
  ...
  function bindWelcomeButtons(){
    ...
    if (login && !login.__safeLoginBridge) {
      ...
      login.addEventListener('click', function(e){ e.preventDefault(); e.stopPropagation(); showAuth('login'); }, true);
    }
    const signup = $('welcomeSignupBtn');
    if (signup && !signup.__safeLoginBridge) {
      signup.__safeLoginBridge = true;
      signup.addEventListener('click', function(e){ e.preventDefault(); e.stopPropagation(); showAuth('signup'); }, true);
    }
  }
  ...
  function setPanel(name) { name = normalize(name); syncNav(name); panels.forEach(([key, id]) => { const active = key === name; $('panel' + id)?.classList.toggle('on', active); $('tab' + id)?.classList.toggle('on', active); }); $('panelContact')?.classList.remove('on'); showSheet(); try { window.App?.openSheet?.(); } catch (e) {} }
  ...
  window.UIManager = { openSheetPanel: setPanel, openMessagesInbox, closeFloatingPanels: closeFloating, hideSheetCompletely: hideSheet, showSheetAgain: showSheet, syncBadge, showAuth, submitAuth, ensureSupabase };
  ...
  setTimeout(bindWelcomeButtons, 300); setTimeout(bindWelcomeButtons, 1000); setTimeout(bindAuthButton, 300); setTimeout(bindAuthButton, 1000);
})();

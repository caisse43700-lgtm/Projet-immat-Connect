...
SW                     : v428 · app.css v61 · narrator.js v9 · messages.js v40 · messages.css v7 · calls.js v22 · audio-manager.js v9 · ui.js v16 · bus.js v51 · immat-consciousness.js v2 · immat-nexus.js v14 · immat-copilot.js v7
...
| D23 | **NO-GO sur la généralisation du « catalogue de faits autorisés » (`core/immat-facts.js`).** Expérience scientifique E2 (migration mesurée de `SIGNAL_VEHICULE`) + E3 (2ᵉ fait `SIGNAL_ROUTE`, moteur réutilisé sans branche par-fait) sur la branche `local/merge-to-main` (commits `a13617c` + `1a3cd96`, **jamais fusionnés sur main**). Résultat brut, moteur inclus : **E2 = +29 LOC, E2+E3 = +45 LOC** → le catalogue **NE RÉDUIT PAS le code**, il **déplace la complexité vers de la déclaration** (métadonnées verbeuses). Ce qui est réellement gagné (mesuré) : **change-locality 6→2**, duplication supprimée, **dérives de vocabulaire éliminées**, tests verts, moteur générique réutilisé. Le prototype reste **sur main comme preuve expérimentale**, non généralisé. **Règle future — n'utiliser un catalogue de faits QUE si les 4 conditions sont réunies : (1) plusieurs consommateurs réels du même fait ; (2) forte duplication existante ; (3) risque de dérive prouvé ; (4) gain de locality > coût d'indirection. Sinon : garder le code direct (plus simple).** Ne pas rediscuter sans nouvelle densité de consommateurs. **Revue critique externe (validée) → principe raffiné : la bonne règle n'est PAS « tout est un fait » mais « centraliser seulement ce qui dérive réellement » ; si un besoin réapparaît, repartir d'un simple *catalogue de taxonomies/vocabulaire partagé* (variante C), pas du catalogue de faits complet. Condition ①=≥3 consommateurs ; + règle de refus explicite.** **CLOS/DÉFINITIF (YAGNI : aucune mini-spec tant qu'aucun cas réel). Réouverture UNIQUEMENT via un dossier séparé « taxonomie partagée » si une même taxonomie nourrit ~5-8 consommateurs réels (vocal/boutons/dashboard/notifs/tests/aide/traduction/export/doc) — jamais la théorie « tout est un fait ».** **Papier complet : `docs/research/D23-verdict-catalogue-faits-nogo.md`.** | Archi / expérience E2-E3 | 2026-07-01 |
...
| Date | Auteur | Résumé |
|---|---|---|
| 2026-07-01 | IA session | **Expérience E2+E3 « catalogue de faits » → VERDICT NO-GO (décision D23)**. Migration mesurée de `SIGNAL_VEHICULE` (E2, commit `a13617c`) puis `SIGNAL_ROUTE` (E3, commit `1a3cd96`) vers `core/immat-facts.js`, sur branche `local/merge-to-main`, **jamais fusionnée sur main**. Chiffres bruts moteur inclus : **E2 = +29 LOC · E2+E3 = +45 LOC** → le catalogue ne réduit PAS le code, il déplace la complexité vers de la déclaration verbeuse. Gains réels mesurés : change-locality **6→2**, duplication supprimée, dérives de vocabulaire éliminées, tests verts (npm 177+3 · proto 31 · ange-v2 242), moteur générique réutilisé sans branche par-fait. Décision PO : **NO-GO généralisation**, on ne fusionne pas E2/E3, le prototype reste sur main comme preuve. Règle future consignée en D23 (catalogue uniquement si plusieurs consommateurs + forte duplication + dérive prouvée + locality > indirection). |

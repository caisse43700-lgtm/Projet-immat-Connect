...
SW                     : v462 · app.css v61 · narrator.js v9 · messages.js v40 · messages.css v7 · calls.js v22 · audio-manager.js v9 · ui.js v16 · bus.js v51 · immat-consciousness.js v2 · immat-nexus.js v14 · immat-copilot.js v8
...
| Date | Auteur | Résumé |
|---|---|---|
| 2026-07-02 | IA session | **v462 — clé feature FANTÔME `'signaler'` retirée (bug compte GARDIEN) + tout blocage feature tracé au HUD (SW v462)**. Rapport PO : « ouvre signaler » à la voix marche sur le compte classique mais PAS sur le compte gardien (v461 confirmée au HUD, routage « requête traitée → ok » mais aucun `🔊 ange-fait` ni ✅). Diagnostic : l'entrée `_OPEN` signaler portait `feat:'signaler'` — une clé qui N'EXISTE PAS dans FEATURE_REGISTRY (1 seule occurrence dans tout le code = cette entrée) et que le chemin CLIC ne vérifie jamais (`navSignaler` : 0 requireFeature). Sur l'appareil gardien, des flags locaux (`ic_feature_flags`) / flotte (`feature_config`) hérités des tests Dashboard font répondre `featureStatus('signaler')` `{enabled:false}` → blocage SILENCIEUX de la voix seule. Fix : (1) `feat:null` sur l'entrée signaler = parité stricte clic/voix ; (2) plus JAMAIS de blocage muet — la branche feature-OFF logge désormais `⛔ <clé> bloqué : feature « <feat> » OFF (<by>)` au HUD (tout futur blocage se nomme). IC_BUILD v462, SW v461→v462. Tests ange-v2 347 (+2 : parité feat:null, trace ⛔) ; e2e nav-invariant 4/4 en Chromium réel. À VÉRIFIER PO : MAJ jusqu'à `ANGE DEBUG v462` au HUD, retester « ouvre signaler » sur le compte gardien ; si encore bloqué, la ligne ⛔ nommera le flag exact à nettoyer. |
| 2026-07-02 | IA session | **Navigation V2 VÉRIFIÉE EN NAVIGATEUR RÉEL + test e2e permanent (D3)**. Rapport PO « depuis Activité, ouvre signaler ne marche pas » → au lieu de deviner : reproduction dans Chromium/Playwright (serveur local, appScreen forcé actif). RÉSULTAT : sur v461, TOUT PASSE — Activité→« ouvre signaler »→`panelAltet VISIBLE(419px)` + sigStep1 actif ; le scénario PROFOND (dans la catégorie Aide, `actCat:'aide'`) passe aussi ; toutes les transitions croisées (messages/appels/gps/activité/signaler) affichent exactement UN panneau. Conclusion : le code v461 est correct ; l'échec téléphone = très probablement version en cache antérieure (le HUD affiche la version : vérifier ≥ v461). Livré : `e2e/nav-invariant.spec.js` (4 tests, tournera en CI via e2e.yml) — le bug de classe « panneau invisible » est désormais une régression détectable. |
| 2026-07-02 | IA session | **NAVIGATION V2 (v461) — le propriétaire unique `App.navigate(intent)` (SW v461)**. GO PO suite à la réflexion « l'affaire des panneaux ». Doctrine gravée : *tout état perçu par l'utilisateur a un propriétaire unique, un invariant nommé, une preuve observable*. Implémenté : (1) **`App._NAV`** (7 destinations : gps/messages/appels/activite/signaler/reglages/dashboard, avec intents profonds `category`/`step`/`view`/`filter`) + **`App.navigate(dest,opts)`** = mémorise l'INTENTION (`App._lastNavIntent`) → délègue aux fonctions propriétaires existantes → impose l'invariant « un seul panneau visible » en UN point (`_forcePanel`) ; (2) **le CLIC passe par navigate** (`_navToggle` → `App.navigate(key,{source:'click'})`, toggle/anti-double conservés) ; (3) **la VOIX passe par navigate** (les 7 `_OPEN.open()` réécrits + `_tryCategory` signaler/activité) ; (4) **correcteur d'invariant amélioré** : actif EN PROD (silencieux), tolérance 2 ticks (transitions animées), correction vers l'INTENTION mémorisée (plus d'heuristique), compteur `_stats.nav`. Clic et voix = UN SEUL chemin → toute divergence future est impossible par construction. IC_BUILD v461. SW v460→v461. Tests ange-v2 345. Reporté : test Playwright de l'invariant (D3, après recette T11). |
| 2026-07-02 | IA session | **Ange v460 — INVARIANT universel « un panneau ouvre, l'autre ferme » à la voix (SW v460)**. Demande PO : la règle des clics (un seul panneau visible) doit valoir pour la voix. Complété : GPS et Réglages ouverts à la voix appliquent désormais `_forcePanel` (les 2 derniers trous) → TOUTES les destinations vocales (GPS/Messages/Appels/Activité/Signaler/Réglages) masquent explicitement les frères. + Assertion permanente (ticker debug) : ≥2 panneaux visibles simultanément = état impossible → HUD `🚨 ASSERT: N panneaux visibles` + auto-correction (`_forcePanel` sur le dernier). IC_BUILD v460. SW v459→v460. Tests ange-v2 342. |
| 2026-07-02 | IA session | **Ange v459 — FIX « ouvre signaler depuis Activité » : forçage symétrique des panneaux (SW v459)**. Capture PO (HUD v458 ✓) : dans Activité/Aide, « Ouvre signaler » traité (ok=6) mais l'écran RESTE sur Activité. Cause : `navActivite` force son panneau à la main (display+classe) ; `openReport` ne masquait PAS ce frère encore affiché → Signaler s'ouvrait DERRIÈRE ; la vérification v458 était trompée (la cible a une hauteur même recouverte). Fix : helper unique `App._forcePanel(pid)` = masque explicitement les 5 panneaux frères + affiche la cible + `openSheet` — utilisé par `openReport`, `navMessages`, `navAppels` et par le forçage post-vérification ; vérification renforcée (un FRÈRE encore visible = échec détecté → forçage). IC_BUILD v459. SW v458→v459. Tests ange-v2 339. |
| 2026-07-02 | IA session | **Ange v458 — PREUVE DE VISIBILITÉ + version dans le HUD (SW v458)**. Capture PO : « Ouvre signaler » traité (ok=1) mais CARTE toujours à l'écran — impossible de distinguer cache périmé (v456) vs fix v457 insuffisant. Fix d'observabilité : (1) **`IC_BUILD` affiché dans l'en-tête du HUD** (« ANGE DEBUG v458 ») — règle : tenir en phase avec CACHE_NAME à chaque release (test automatique l'exige) ; (2) **vérification de visibilité** : 0,7 s après chaque ouverture vocale, contrôle réel (`getBoundingClientRect().height>40` + display/visibility) du panneau cible → HUD `✅ panelX visible` ou `🚨 ouverture invisible → forçage` + **forçage manuel complet** (masque les panneaux frères, affiche la cible, `openSheet`) — le remède intégral de navActivite appliqué en filet. SW v457→v458. Tests ange-v2 337 (dont test de cohérence IC_BUILD=CACHE_NAME). |
| 2026-07-02 | IA session | **Ange v457 — FIX « ça ouvre pas signaler/messages/téléphone » : feuille forcée visible (SW v457)**. Cause trouvée par comparaison : `navActivite` force explicitement `openSheet()` (commentaire historique « panel() peut être swallowé par patches ui.js ») → Activité marchait à la voix ; `navMessages`, `navAppels` et `openReport` ne le faisaient PAS → au doigt la feuille est déjà dépliée (ça passe), à la voix depuis la carte le contenu changeait dans une feuille REPLIÉE/invisible = « rien ne s'ouvre ». Fix à la source : les 3 fonctions forcent désormais `openSheet()` après `panel()` (même remède que navActivite, bénéficie à TOUS les appelants). SW v456→v457. Tests ange-v2 334. |
| 2026-07-02 | IA session | **Ange v456 — SIGNALEMENT COMPLET 100 % VOCAL : choix contextuels (SW v456)**. Demande PO : naviguer à la voix jusqu'au FOND (fonctionnalités → catégories → CHOIX) et envoyer un signalement entier sans toucher. Audit du registre : `roadReport(type)` et `assist(type)` sont des actions AUTONOMES (GPS+envoi+confirmation) → le trou était la sélection vocale des choix dans le contexte Signaler (« accident » tombait sur « Répète autrement »). Nouveau `_tryChoice(msg)` (avant `_tryCategory`) : contexte `_navView==='signaler'` + mot court (≤4 mots) → route (accident/bouchon/obstacle/travaux/contrôle/danger → `roadReport`) et aide (panne/carburant/batterie/moteur/incendie/perdu → `assist`) → « Envoyé » (WAV) + retour veille. Mots dérivés du registre `_MENU.signaler.*`. + `_tryCategory('signaler')` garantit la feuille VISIBLE (`openReport()` d'abord — l'étape s'ouvrait derrière l'écran = « rien ne se passe »). Boucle complète démontrable : « Hé Ange → ouvre les signalements » → « route » → « accident » = signalement envoyé, 3 phrases, 0 toucher. Véhicule déjà complet via « signale un pneu » (rail+confirmation). SW v455→v456. Tests ange-v2 331. |
| 2026-07-02 | IA session | **Ange v455 — PURGE speechSynthesis en session vocale (dernier parasite, SW v455)**. HUD terrain v454 : la boucle cible FONCTIONNE (`requête traitée → veille`, stats det=1 ok=1, auto-réparation capture en 1 s) MAIS une tentative de synthèse résiduelle partait APRÈS le retour en veille (`_speakVoiceResult`/`_speakAnswer` différés, flags déjà réinitialisés → 3 réessais condamnés + bips parasites + collision audio-capture). Principe v455 : **en session « Ange », AUCUNE speechSynthesis** (mur M1 : toujours avalée) — le son vient exclusivement de la banque WAV + bips. `_speakAnswer` en session → bip `ok` + visuel ; filet `_speakVoiceResult` réservé aux sessions TAP. SW v454→v455. Tests ange-v2 325. |
| 2026-07-02 | IA session | **ANGE VOCAL v454 — GO revue ultime : certitude auditive + cohérence d'états + mesure (SW v454)**. Plan validé (12 points) implémenté intégralement : ① **Banque audio = charte vocale** générée par script versionné `tools/make-voices.sh` (10 WAV SVOX : J'écoute · Oui ? · C'est fait · Envoyé · Annulé · Répète autrement · Message ? · Disponible à l'arrêt · Micro instable utilise le bouton · Hors ligne) — greet raccourci « J'écoute » (décision), phrases « J'ouvre… » retirées ; ② **son dédié retour veille** (earcon `sleep`, 2 notes descendantes) ; ③ **LLM COUPÉ en session wake** → « Répète autrement » + veille (« Hors ligne » si offline) ; ④ **dictée vide/bruit → veille immédiate** ; ⑤ **anti-écho** (nos phrases captées ignorées) + **détection gelée pendant toute voix de l'app** (GPS) ; ⑥ **Wake Lock armé avec la veille** (opt-out `ic_wake_lock`, toggle Réglages) ; ⑦ **FSM-ombre** `_vGo` (VEILLE/DETECTE/GREET/DICTEE/ROUTAGE/CONFIRMATION/FEEDBACK/OFF) loggée HUD ; ⑧ **assertions états impossibles** (ticker 2 s : double reco, micro pendant voix, arrière-plan, zombie >60 s) ; ⑨ **politique audio** : appel Agora → veille suspendue/reprise + bip ; GPS non forcé coupé pendant session Ange ; ⑩ **stats recette** (`_stats` det/rep/incompris/watchdog/capture/ok en pied de HUD) ; ⑪ **`drivable` minimal** (réglages+dashboard refusés en conduite → « Disponible à l'arrêt ») ; ⑫ **message libre INTERDIT en Mode Volant** (non relisible sans TTS) + suivi « Message ? » hors Volant (`_followUp`) ; + « Envoyé »/« Annulé » vocaux, mode dégradé N1 (« Micro instable » après 3 incidents), watchdog 18 s, double signal supprimé, wake officielle « Hé Ange » (coaching). SW v453→v454. Tests ange-v2 323, npm 177+3, proto 18. NB : Pages en incident (déploiements en file annulés) — le déploiement partira dès rétablissement. |
| 2026-07-02 | IA session | **ANGE VOCAL V3 — AUDIT COMPLET + modèle ONE-SHOT (SW v453)**. Audit de toute la bataille vocale (12 bugs, cf. SESSION-CONTINUATION) → architecture cible PO : « Ange » → « Je t'écoute » (voix audio) → UNE requête → exécution + « C'est fait » (voix audio) → **STOP, retour en veille** → prochain « Ange » recommence. Implémenté : (1) `_voiceTurn` : requête traitée sans pending/choices en session « Ange » → `_convoStop()` (réarme la veille) — FIN de la conversation continue fragile (dictées en boucle, timeouts, pauses) ; rails à choix et confirmations gardent l'écoute un tour ; (2) `_tryOpen`/`_tryCategory` : plus d'auto-reprise en session Ange + confirmation vocale **« C'est fait »** (`audio/ange-fait.wav`, SVOX) ; (3) « Ange » SEUL → détection 0,5 s (vs 0,9) ; (4) **CHIEN DE GARDE** : écoute sourde 25 s sans résultat → redémarrage auto (cause majeure des « je répète Ange, rien ») ; (5) préchargement des 3 voix. SW v452→v453. Tests ange-v2 304. |
| 2026-07-02 | IA session | **Ange — VOIX AUDIO CONFIRMÉE AUDIBLE terrain 🎉 + voix améliorée (SVOX Pico) + relance plus rapide (SW v452)**. PO : « la voix est très bizarre » = IL L'ENTEND → le contournement Web Audio fonctionne. Améliorations : (1) voix régénérée avec **SVOX Pico fr-FR** (naturelle, vs espeak robotique) — `audio/*.wav?v=2` ; (2) relance du wake après fin d'écoute **0,7 s → 0,35 s** (moins de trous morts où les « Ange » se perdent → latence perçue réduite ; le gros de la latence restante = iOS transcrit mal « Ange » isolé, plusieurs tentatives nécessaires) ; (3) reste à diagnostiquer : « ouvre activité » sans effet (besoin capture HUD du moment précis). SW v451→v452. Tests ange-v2 299. |
| 2026-07-02 | IA session | **Ange — « Je t'écoute » EN VRAIE VOIX via Web Audio (contourne le blocage iOS) (SW v451)**. PO : « je veux qu'il me dise j'écoute quand je dis Ange ». Solution : la synthèse iOS est bloquée en chaîne vocale, MAIS le canal Web Audio (bips) passe toujours → **voix PRÉ-ENREGISTRÉE** jouée par ce canal. Fichiers `audio/ange-ecoute.wav` (« Je t'écoute », 1,1 s) + `audio/ange-oui.wav` (« Oui ? ») générés (espeak-ng fr). `_playVoice(name,done)` : fetch+decodeAudioData+BufferSource sur le même AudioContext que les earcons ; cache mémoire `_vBufs` + préchargement à l'autorisation (`_voicePrefetch`) ; repli bip si échec. Branché : wake → `ange-ecoute` puis micro à la fin de la voix ; « Ange » redit en chaîne vocale → `ange-oui`. SW cache les 2 WAV. SW v450→v451. Tests ange-v2 299. NB : voix synthétique (espeak) — à remplacer plus tard par un enregistrement humain/TTS de qualité (même mécanisme). |
| 2026-07-02 | IA session | **Ange — CLÔTURE de la bataille TTS : mur iOS confirmé, mode vocal = BIP + action rapide (SW v450)**. HUD IMG_6600-02 (v449) : même l'astuce blip échoue (« réessai 2 (blip d'abord) » → « voix indisponible »). VERDICT DÉFINITIF : WebKit exige une activation utilisateur (toucher récent) pour la synthèse vocale — TOUTE chaîne 100 % vocale est muette, quel que soit le contournement (ordre, cancel/resume, blip, prime). Décision produit : `voiceCommand(cmd,fromWake)` — en chaîne vocale (`fromWake`), plus AUCUNE tentative TTS condamnée (perdait 3-4 s) : **bip immédiat = « je t'écoute » + micro ouvert en 400 ms** ; drapeau `_voiceNoTTS` → réponses limitées à 1 essai TTS + `_icTts='done'` à l'échec (libère le micro sans attendre). La voix parlée reste ENTIÈRE dans les sessions démarrées au TAP (micro/orbe/bouton), là où Apple l'autorise. Latence vocale totale ≈ 1,3 s (vs ~5 s). Voie future pour la voix en chaîne vocale : TTS serveur (audio HTML non soumis à l'activation) ou app native. SW v449→v450. Tests ange-v2 293. |
| 2026-07-02 | IA session | **Ange — VERDICT iOS : synthèse vocale BLOQUÉE sans toucher récent + astuce blip (SW v449)**. HUD IMG_6595-99 (v448) : expérience PROPRE — ordre parfait (greet AVANT dictée), micro fermé pendant les 3 essais… et TOUTES les phrases avalées quand même (greet ET « Oui, je t'écoute » post-dictée). **Conclusion ferme : WebKit exige une activation utilisateur (toucher récent) pour `speechSynthesis.speak()` en PWA — une chaîne 100 % vocale n'en a pas → phrases jetées ('starting' ne devient jamais 'speaking'). Au TAP ça parle (activation), à la voix jamais. Web Audio (bips) passe toujours.** Dernière carte : astuce bascule audio — dans `_speakRetry`, jouer un mini-son Web Audio (qui passe) juste avant de reparler (250 ms) pour forcer la sortie en lecture ; greet limité à 2 essais (latence micro). Si l'essai 2 avec blip échoue aussi → repli définitif : bip = « je t'écoute », voix parlée uniquement dans les sessions démarrées au tap ; vraie solution = TTS serveur (audio HTML, non soumis à l'activation) ou app native. SW v448→v449. Tests ange-v2 293. |
| 2026-07-02 | IA session | **Ange — FIX ordre greet/micro : le micro s'ouvrait AVANT « Je t'écoute » (SW v448)**. HUD IMG_6591-94 (v447) : schéma 100 % reproductible — `dictée : démarrage` (600 ms) AVANT `greet lancé` (800 ms) → les 3 réessais de voix TOUS avalés par le micro ouvert → « voix indisponible → bip ». Bug d'ordre introduit en v447 (greet retardé à 800 ms mais boucle micro restée à 600 ms). Fix : **SÉRIALISATION** — le greet part d'abord, la boucle d'ouverture micro ne démarre QU'ENSUITE (imbriquée dans le même timeout), n'ouvre qu'après la fin réelle de la voix (`_ttsBusy`, plafond 9 s). + Garde d'instance TTS (`window._icTtsU`) : le onerror tardif d'une vieille phrase annulée ne clobber plus l'état de la nouvelle. SW v447→v448. Tests ange-v2 292. |
| 2026-07-02 | IA session | **Ange — « Je t'écoute » AVALÉ par iOS → parler avec RÉESSAI (SW v447)**. PO : toujours pas de « Je t'écoute » malgré la course voix/micro réglée. HUD : l'écart de 3 s = plafond d'attente, PAS la phrase — `speak()` appelé mais l'utterance ne DÉMARRE jamais (onstart jamais émis) : après une écoute micro, la session audio iOS est encore en mode enregistrement et AVALE la 1re synthèse. Fix : `_speakRetry(txt)` — vérifie 1,1 s après `speak()` que l'état est passé à speaking/done ; sinon `cancel()+resume()+re-speak` (3 essais), puis bip de repli. Branché sur : accusé « Je t'écoute » (délai 450→800 ms), `_speakAnswer` (réponses), ré-accueil « Oui, je t'écoute ». SW v446→v447. Tests ange-v2 290. |
| 2026-07-02 | IA session | **Ange — VICTOIRE course voix/micro confirmée + « Ange » redit = ré-accueil (SW v446)**. HUD IMG_6586-87 (v445) : « ange » reconnu du 1er coup, détection, `speak()` puis dictée **3 s après** → « Je t'écoute » a enfin le temps d'être prononcé (fix _ttsBusy validé terrain). Dernier détail : l'utilisateur redit « Ange » dans la conversation → partait dans la chaîne de commandes jusqu'au LLM. Fix : « Ange » (et variantes) redit en pleine conversation = simple ré-accueil vocal « Oui, je t'écoute » + reprise d'écoute, jamais transmis aux commandes/LLM. SW v445→v446. Tests ange-v2 289. |
| 2026-07-02 | IA session | **Ange — FIX de la course voix/micro : « Je t'écoute » tué à la naissance (SW v445)**. HUD IMG_6581-85 : flagrant délit `speak()` et `dictée : démarrage` à la MÊME seconde. iOS met ~1 s à VRAIMENT démarrer la synthèse ; on vérifiait `speechSynthesis.speaking` trop tôt (encore false pendant le démarrage) → le micro s'ouvrait immédiatement et tuait la phrase naissante (conflit STT/TTS) → « Je t'écoute » et les réponses jamais entendus quand déclenchés par la voix. Fix : `speak()` trace l'état réel (`window._icTts` = starting→speaking→done via onstart/onend) ; nouveau `_ttsBusy()` (couvre 'starting' <2,5 s + speaking) utilisé aux 4 points d'attente : boucle micro de `voiceCommand` (plafond 7 s), `_convoResume`, `_armConfirm`, `_speakVoiceResult`. Latence détection restante = iOS transcrit mal « Ange » isolé (bonjour/et… avant enfin « ange ») → coaching « Hé Ange » / « Ange, + commande ». SW v444→v445. Tests ange-v2 288. |
| 2026-07-02 | IA session | **Ange — débounce de VRAIE fin de phrase + feedback immédiat + reprise audio-capture (SW v444)**. HUD IMG_6579-80 : « Ange, ouvre l'activité » coupé à « ou » (le minuteur 0,9 s partait du 1ER mot, pas de la fin de phrase) → commande jetée, « Je t'écoute » pendant que l'utilisateur parlait encore, dictée vide, latence perçue. Fixes : (1) minuteur **réarmé à chaque nouveau résultat** → agit 0,9 s après la FIN de la phrase → la commande complète est captée et exécutée directement ; (2) **bip + orbe 'hear' dès la 1re détection** (« je t'ai entendu, finis ta phrase ») → latence perçue réduite ; (3) `erreur: audio-capture` (collision de micros en transition) → **réessai auto 1,2 s** ; (4) seuil commande 4→3 chars (« gps »). SW v443→v444. Tests ange-v2 286. |
| 2026-07-02 | IA session | **Ange — DERNIÈRE PIÈCE : « Ange » seul mal transcrit par iOS (SW v443)**. HUD IMG_6577 post-v442 : `audio ON` ✅ + `entendu:` arrive enfin ✅ (fix fantômes validé) MAIS l'utilisateur dit « Ange » et iOS transcrit **« non »** → le mot isolé (1 syllabe nasale) est mal reconnu par la dictée Apple. Fixes : (1) matcher élargi (anges/l'ange/lange/angel/angie/anj…) ; (2) **commande dans le même souffle** : « Ange, ouvre l'activité » → délai 0,9 s pour laisser la phrase se terminer, puis `voiceCommand(cmd)` exécute DIRECTEMENT via `_voiceTurn` (sans « Je t'écoute ») ; (3) coaching UI : dire « **Hé Ange** » ou « Ange, ouvre l'activité » (bien mieux transcrit qu'« Ange » seul). `_wakePend` nettoyé dans `_wakeStop`. SW v442→v443. Tests ange-v2 283. |
| 2026-07-02 | IA session | **Ange — BUG RACINE TROUVÉ via HUD terrain : instances fantômes du micro (SW v442)**. Captures IMG_6571-75 : le micro DÉMARRE (ÉCOUTE), mais AUCUN « entendu: » + « erreur: aborted » ×2 + micro encore tenu à l'écran d'accueil. Cause : (1) `_primeTTS` sur le 1er tap partait PENDANT le démarrage du micro → iOS avorte l'écoute (conflit STT/TTS) ; (2) PAS d'hygiène d'instances : le `onerror` d'une vieille instance avortée écrasait `_wakeRec=null` → instances FANTÔMES qui recevaient les « Ange » dans le vide et gardaient le micro pour toujours. Fixes : handlers vérifient l'instance active (`this._wakeRec!==rec → return`) ; `_wakeStop` nettoie tout + `abort()` ; registre `_recTrack` de toutes les instances (wake+dictée+confirmation) ; `_hardStopMic` avorte TOUT le registre (fix micro qui reste allumé) ; `_primeTTS` reporté si micro actif ; HUD enrichi (audio ON, dictée, prime). SW v441→v442. Tests ange-v2 281. |
| 2026-07-02 | IA session | **Ange — BUILD DIAGNOSTIC vocal + fix conflit prime/micro (SW v441)**. Vocal « Ange » toujours KO → instrumentation pour obtenir la vérité terrain : `_wakeDbg()` = petit afficheur vert en bas de l'écran qui trace en direct le wake (start/onstart/entendu/ANGE détecté/erreur/fin/relance) + `voiceCommand`→speak. Fix soupçonné : le déblocage voix (`_primeTTS`) juste avant `rec.start()` entrait en conflit (STT+TTS simultané interdit iOS) → sorti du bouton « Autoriser » ; le déblocage voix se fait maintenant au TOUT 1er contact tactile global (listener `pointerdown`), le bouton ne fait plus QUE `close()`+`toggleAngeWake(true)`. À FAIRE : capture du HUD pour diagnostiquer. `_dbgOn:true` (à repasser false après diag). SW v440→v441. Tests ange-v2 276.
| 2026-07-02 | IA session | **Ange — audit vocal iOS + bouton « Autoriser » (1 geste → « Ange ») (SW v440)**. Audit approfondi : le vrai « dis Ange » mains-libres persistant = IMPOSSIBLE en PWA (3 murs Apple : MUR A `SpeechRecognition.start()` exige un geste → wake non redémarrable seul ; MUR B `speechSynthesis` déclenché par la voix bloqué ; MUR C pas de STT+TTS simultané). Décision PO : accepter UN geste = un bouton « Autoriser » à l'ouverture. `_wakeHintHTML` refait : le bouton, DANS le tap, (1) `_primeTTS()` (vraie micro-utterance vol 0.05 → débloque la voix), (2) `close()`, (3) `toggleAngeWake(true)` (arme+démarre l'écoute « Ange » dans le geste). Toujours visible (re-toucher réarme si iOS coupe le micro). Ensuite « Ange » → orbe + « Je t'écoute » (voix débloquée) + écoute. SW v439→v440. Tests ange-v2 274.
| 2026-07-02 | IA session | **Ange — catégories CONTEXTUELLES + ordre des handlers assaini (SW v439)**. Demande PO : « route / véhicule / aide / stationné » doit ouvrir la catégorie DANS la fonctionnalité en cours. Nouveau `_tryCategory(msg)` : lit `App._navView` → si `signaler` = étape de signalement (`sigStepRoute/Vehicle/Aide/Station`), sinon = catégorie d'activité (`navActivite`+`openActivityCat`). Ignore les commandes d'action explicites (signale/appelle/envoie/active). Ordre `send()` remis d'aplomb : actions (message/appel/signal/gouvernance) → catégorie contextuelle → `_tryOpen` → guide → menu. Confirmation vocale (« J'ouvre l'activité route »). SW v438→v439. Tests ange-v2 268→274.
| 2026-07-02 | IA session | **Ange orbe — « Je t'écoute » sur « Ange » : libère le micro avant de parler (SW v438)**. Piste : les messages proactifs S'ENTENDAIENT (donc TTS pas bloqué hors geste une fois débloqué) → le vrai coupable du « Je t'écoute » muet sur « Ange » = le micro du wake encore actif (iOS ne fait pas écoute+voix en même temps). Fix `voiceCommand` : `_wakeStop()` d'abord, puis « Je t'écoute » DIFFÉRÉ ~450 ms (micro libéré), puis micro rouvert ≤1,6 s. Le wake s'arme via `startVoice` après le 1er tap « Parler à Ange ». SW v437→v438. Tests ange-v2 268. Si toujours muet sur « Ange » → mur iOS (natif requis), le tap reste la voie sûre.
| 2026-07-02 | IA session | **Ange orbe — diagnostic iOS confirmé + bouton « Parler à Ange » (SW v437)**. Preuve terrain : la voix marche quand la session démarre par un GESTE (tap micro → « Je t'écoute », ouvre l'activité, répond à voix haute) mais reste muette quand déclenchée par le mot « Ange » (Safari interdit voix/micro hors geste — vrai « Ange » mains-libres = app native requise). Décision : le bouton `_wakeHintHTML` (« Activer le micro ») DÉMARRE désormais la conversation dans le geste (`_softHide()`+`voiceCommand()`, onclick synchrone → la voix passe) au lieu d'armer le wake muet. Renommé « 🎙️ Parler à Ange ». Après ce tap, conversation 100 % mains-libres (répond + rouvre le micro). SW v436→v437. Tests ange-v2 266. RESTE : « ouvre route » non géré (tombe sur le LLM qui dit « je ne peux pas »).
| 2026-07-02 | IA session | **Ange orbe — fix « Je t'écoute » muet : double cancel() retiré (SW v435)**. Cause réelle : `voiceCommand` appelait `speechSynthesis.cancel()` juste AVANT `speak()`, qui refait un `cancel()` en interne → double cancel collé au speak = bug iOS qui fait « sauter » l'utterance (les messages proactifs, eux, s'entendaient car un seul cancel). Retrait du cancel superflu dans `voiceCommand` (speak() gère seul l'interruption). SW v434→v435. Tests ange-v2 265→266. |
| 2026-07-02 | IA session | **Ange orbe — voix débloquée (iOS) + bip unique + « ouvre X » navigue (SW v434)**. Retour PO (orbe OK mais) : pas de « Je t'écoute », « ouvre l'activité » sans effet, et un bip toutes les 5-10 s. Fixes : (1) `_primeTTS()` au 1er contact tactile (utterance silencieuse) → débloque la synthèse iOS pour que la voix déclenchée par « Ange » (hors geste) soit prononcée ; (2) earcon `listen` joué **1×/session** (`startVoice(resume)` + `_convoResume`→`startVoice(true)`) → plus de bip répété à chaque réouverture du micro ; (3) `_tryOpen` passe **avant** `_tryGuide` dans `send()` → « ouvre l'activité/messages/GPS » **navigue** au lieu d'afficher un sous-menu dans la fiche masquée. SW v433→v434. Tests ange-v2 260→265. |
| 2026-07-02 | IA session | **Ange orbe — fix « plus d'orbe » (garde anti-blocage, SW v433)**. Régression v432 : le garde `_orbStarting` restait coincé à `true` si `speechSynthesis.speaking` restait bloqué (bug iOS), donc `voiceCommand` sortait immédiatement → l'orbe n'apparaissait plus. Fix : orbe affiché **immédiatement** (`_setOrb('listen')` sans dépendre d'une voix) ; ouverture du micro via une boucle **plafonnée à ≤1,4 s** (`_w<1400`) qui relâche TOUJOURS `_orbStarting` et lance `startVoice` même si la synthèse reste coincée. SW v432→v433. Tests ange-v2 259→260. |
| 2026-07-02 | IA session | **Ange orbe — fin des déclenchements multiples + « Je t'écoute » fiable (SW v432, déployé)**. Retour PO (v431 OK mais) : « Parlez maintenant » s'ouvrait plusieurs fois et « Je t'écoute » n'était pas dit. Causes : (1) le wake « Ange » fire plusieurs fois (résultats intermédiaires) → plusieurs `voiceCommand`/`startVoice` ; (2) double `speechSynthesis.cancel()` juste avant `speak()` (bug iOS cancel→speak) ; (3) `speak()` muet si voix GPS `S.voice` off. Fix : garde `_wakeFired` (wake ne déclenche qu'1×) + garde `_orbStarting` (voiceCommand idempotent) ; « Je t'écoute » différé 200 ms (libère le micro + évite cancel→speak) ; `speak(txt,force,ignoreMute)` → l'orbe parle même si voix GPS coupée (`_speakAnswer`, confirmations, accusé) ; toast « Parlez maintenant » masqué en mode orbe. SW v431→v432. Tests ange-v2 256→259. |
